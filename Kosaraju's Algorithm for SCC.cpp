...
vi G[MAXV], Grev[MAXV];
bool explored[MAXV];
ll leader[MAXV], finish[MAXV], order[MAXV], t = -1, parent = 0, V, E;

// running DFS on the reverse graph
void dfs_reverse(ll i) {
    explored[i] = true;
...
bool stronglyConnected(ll u, ll v)    {
...
int main()  {
    ...
    scanf("%lld %lld", &V, &E); // Enter the number of vertices & edges
    ...
    scanf("%lld", &Q); // Enter number of SCC queries
    while(Q--)  {
        scanf("%lld %lld", &u, &v);
